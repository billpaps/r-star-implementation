...
public class RTree {
    ...
    private double[][] mbrCalculate(ArrayList<Node> nodes){
        ...
        for(int i = 0;i < dim;i++){
            for(Node node: nodes){
                for (int j = 0; j < 2; j++){
                    if (node.getMbr()[i][j] > mbr[i][j] || mbr[i][j] == 0){
                        mbr[i][j] = node.getMbr()[i][j];
                    }
                    if (node.getChildren().get(j).getMbr()[i][j]< mbr[i][j] || mbr[i][j] == 0){
                        mbr[i][j] = node.getMbr()[i][j];
                    }
                }
            }
    ...
    private ArrayList<Node> sortNodes(Node node, Node newNode, int axis, int mode) {
        ...
        for (int j = 0; j < M; j++) {
            sortedNodes.add(node.getChildren().get(j));
        }
